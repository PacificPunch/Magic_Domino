...
if (count <= 0) {
    ...
    if (variable_global_exists("current_turn") && global.current_turn == "player" && !global.game_over) {
        
        // ВАЖНО: Эта функция уже обновлена в Контроллере и сама проверяет ДВЕ стихии!
        var has_any_valid_move = global.check_has_moves(global.player_hand);
        
        // Если ходов нет — активируем свечение и меняем цвет подложки, призывая игрока взять кость
