...
// А) Логика затемнения недоступных костей (Безопасная проверка)
if (owner == "player" && !global.game_over) {
    // Проверяем, существует ли переменная, прежде чем её читать
    var choice_active = variable_global_exists("choice_mode") ? global.choice_mode : false;
...
// В) Логика режима выбора
if (variable_global_exists("choice_mode")) {
    if (global.choice_mode) {
        // Проверяем, является ли эта кость краем цепочки на столе
        var is_edge = (id == global.left_tile_id || id == global.right_tile_id);
        
        if (owner == "table" && is_edge) {
            draw_col = c_yellow; // Подсвечиваем КРАЯ желтым
        }
        
        // Выбранную кость в руке оставляем белой (она поднимется в Step)
        if (id == global.selected_domino) {
            draw_col = c_white;
        }
    }
}
...
// РАМКА ПОДСВЕТКИ ДЛЯ ИГРОКА (Исправлено: добавлена проверка существования переменной)
if (owner == "player" && variable_global_exists("choice_mode")) {
    // Теперь безопасно проверяем само значение
    if (!global.choice_mode && draw_col == c_white) {
