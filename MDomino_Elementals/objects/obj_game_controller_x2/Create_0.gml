/// obj_game_controller_x2 - Create Event

randomize();
repeat(irandom_range(5, 15)) { random(1); }

// --- 1. ПРЕЖДЕ ВСЕГО ОБЪЯВЛЯЕМ ФУНКЦИИ ---
...
global.resolve_fish = function() {
    ...
    // ВАЖНО: обращаемся к контроллеру x2
    with (obj_game_controller_x2) alarm[3] = 10;
}
...
repeat(150) { // Перемешиваем сильнее, так как массив в 2 раза больше
...
// Раздаем 56 костяшек: 14 игроку, 14 боту, 28 в базар
for (var i = 0; i < 56; i++) {
    var dom = all_dominoes[| i];
    // ВАЖНО: Создаем объект obj_domino_x2
    var inst = instance_create_layer(0, 0, "Instances", obj_domino_x2);
    inst.value1 = dom[0]; 
    inst.value2 = dom[1];
    ...
    // Раздаем по 14 штук
    if (i < 14) { inst.owner = "player"; ds_list_add(global.player_hand, inst); }
    else if (i < 28) { inst.owner = "computer"; ds_list_add(global.computer_hand, inst); }
    else { inst.owner = "bazar"; ds_list_add(global.bazar, inst); }
}
ds_list_destroy(all_dominoes);

// Создаем базар x2
instance_create_layer(200, global.table_center_y, "Instances", obj_bazar_x2);
alarm[0] = 2;
...
global.play_domino = function(dom_id, side) {
    ...
    if (side == "first") {
        ...
        if (side == "right") {
            global.right_count++;
            current_dir = global.right_dir;
            
            // ЛОГИКА ПОВОРОТОВ ПРАВОЙ СТОРОНЫ
            if (global.right_count <= 4) {
                target_dir = "right";
            ...
            } else if (global.right_count <= 17) {
                target_dir = "left";
            } else if (global.right_count <= 22) {
                // Третий поворот: вертикально (противоположно первому)
                target_dir = (global.first_turn_dir == "up") ? "down" : "up";
            } else {
                // Четвертый поворот (21+): возврат в изначальную сторону (вправо)
                target_dir = "right";
            }
            ...
            global.left_count++;
            current_dir = global.left_dir;
            
            // ЛОГИКА ПОВОРОТОВ ЛЕВОЙ СТОРОНЫ
            if (global.left_count <= 4) {
                target_dir = "left";
            } else if (global.left_count <= 6) { 
                if (global.first_turn_dir == "") global.first_turn_dir = choose("up", "down");
                target_dir = (global.first_turn_dir == "up") ? "down" : "up";
            } else if (global.left_count <= 17) {
                target_dir = "right";
            } else if (global.left_count <= 22) {
                // Третий поворот: вертикально (в сторону первого поворота правой ветки)
                target_dir = global.first_turn_dir;
            } else {
                // Четвертый поворот (21+): возврат в изначальную сторону (влево)
                target_dir = "left";
            }
    ...
    if (instance_exists(obj_player_hand_x2)) with (obj_player_hand_x2) arrange_player_hand();
    if (instance_exists(obj_computer_hand_x2)) with (obj_computer_hand_x2) arrange_computer_hand();
    
    global.current_turn = (global.current_turn == "player") ? "computer" : "player";
    
    with (obj_game_controller_x2) alarm[2] = 10;
}