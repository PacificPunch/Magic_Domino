...
if (instance_exists(obj_domino_b_all)) {
    with (obj_domino_b_all) instance_destroy();
}
...
global.resolve_fish = function() {
    ...
    with (obj_game_controller_b_all) alarm[3] = 60; 
}
...
global.play_domino = function(dom_id, side) {
    ...
    with (obj_player_hand_b_all) arrange_player_hand(); // или alarm[0] = 1 в зависимости от твоей логики рук
    with (obj_computer_hand_b_all) arrange_computer_hand();

    global.current_turn = (global.current_turn == "player") ? "computer" : "player";
    with (obj_game_controller_b_all) alarm[2] = 10;
}
...
for (var i = 0; i < 28; i++) {
    ...
    var inst = instance_create_layer(0, 0, "Instances", obj_domino_b_all);
    inst.value1 = dom[0]; 
    inst.value2 = dom[1];
