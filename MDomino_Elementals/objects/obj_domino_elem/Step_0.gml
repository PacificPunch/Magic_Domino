...
// 1. Начало перетаскивания (только если не в режиме выбора направления)
...
if (mouse_check_button_released(mb_left) && dragging) {
    ...
    // Если стол пуст (первый ход в игре)
    if (ds_list_size(global.table_chain) == 0) {
        global.play_domino(id, "first");
        placed = true;
    } 
    else {
        // --- ПРОВЕРКА ВОЗМОЖНОСТИ ХОДА (Цифры + Стихии) ---
        
        // Проверка левого края: (Совпадение цифр) И (Это дубль ИЛИ стихия не конфликтует)
        ...
        // Проверка правого края: (Совпадение цифр) И (Это дубль ИЛИ стихия не конфликтует)
        ...
        // СЛУЧАЙ А: Кость подходит к обоим концам И они разные (требуется выбор игрока)
        if (match_left && match_right && (global.left_end != global.right_end)) {
            global.choice_mode = true;
            global.selected_domino = id;
            y = global.table_center_y + 250; // Поднимаем кость для индикации выбора
            placed = true; 
        } 
        // СЛУЧАЙ Б: Подходит только к левому краю
        else if (match_left) { 
            global.play_domino(id, "left"); 
            placed = true; 
        }
        // СЛУЧАЙ В: Подходит только к правому краю
        else if (match_right) { 
    ...
    // Если кость не подошла (не совпали цифры или заблокировала стихия)
    if (!placed) {
        // Возвращаем в руку на законное место
