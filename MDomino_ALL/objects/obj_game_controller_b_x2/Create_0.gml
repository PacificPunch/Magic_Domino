/// @desc obj_game_controller_b_x2 - Create Event (Режим БЛОК)
...
global.resolve_fish = function() {
    ...
    // Обращаемся к текущему контроллеру режима Блок
    with (obj_game_controller_b_x2) alarm[3] = 60; 
}
...
// Список для базара создаем временно, чтобы не менять логику удаления, 
// но в режиме Блок он останется пустым.
...
// --- 3. СОЗДАНИЕ И РАЗДАЧА КОСТЕЙ (56 ШТУК, РЕЖИМ БЛОК) ---

var all_dominoes = ds_list_create();
for (var v1 = 0; v1 <= 6; v1++) {
    for (var v2 = v1; v2 <= 6; v2++) {
        // Добавляем по 2 копии каждой костяшки (всего 56)
...
for (var i = 0; i < 56; i++) {
    ...
    // Раздаем только первые 28 костяшек (14 + 14)
    if (i < 28) {
        var inst = instance_create_layer(0, 0, "Instances", obj_domino_b_x2);
        inst.value1 = dom[0]; 
        inst.value2 = dom[1];
    ...
    // Оставшиеся 28 костяшек (i >= 28) просто не создаются — в Блоке их нет в игре.
}
ds_list_destroy(all_dominoes);

// В режиме Блок объект obj_bazar_b_x2 не создается.
alarm[0] = 2; // Будильник для первичной отрисовки рук
...
global.play_domino = function(dom_id, side) {
    ...
    if (side == "first") {
        ...
        if (side == "right") {
            ...
            } else if (global.right_count <= 17) target_dir = "left";
            else if (global.right_count <= 22) target_dir = (global.first_turn_dir == "up") ? "down" : "up";
            ...
            } else if (global.left_count <= 17) target_dir = "right";
            else if (global.left_count <= 22) target_dir = global.first_turn_dir;
    ...
    if (instance_exists(obj_player_hand_b_x2)) with (obj_player_hand_b_x2) arrange_player_hand();
    if (instance_exists(obj_computer_hand_b_x2)) with (obj_computer_hand_b_x2) arrange_computer_hand();
    
    global.current_turn = (global.current_turn == "player") ? "computer" : "player";
    
    with (obj_game_controller_b_x2) alarm[2] = 20; // Задержка перед ходом ИИ или проверкой игрока
}