...
for (var i = 0; i < ds_list_size(global.computer_hand); i++) {
    ...
    if (ds_list_size(global.table_chain) == 0) {
        ...
        // Если левый край - Лотос (-1 или стихия LOTOS)
        var can_l_wild = (global.left_end == -1 || global.left_element == ELEMENT.LOTOS);
        ...
        else if (dom.value1 == global.left_end && global.element_conflict[dom.element1] != global.left_element) {
            global.play_domino(dom, "left");
            played = true; break;
        }
        ...
        if (!played) {
            var can_r_wild = (global.right_end == -1 || global.right_element == ELEMENT.LOTOS);
            
            if (can_r_wild) {
                global.play_domino(dom, "right"); 
                played = true; break;
            }
            else if (dom.value1 == global.right_end && global.element_conflict[dom.element1] != global.right_element) {
                global.play_domino(dom, "right"); 
                played = true; break;
            }
