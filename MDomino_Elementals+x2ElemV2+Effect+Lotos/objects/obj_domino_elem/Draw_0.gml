...
if (owner == "player" || owner == "table" || owner == "discarded_player" || global.game_over) {
    show_face = true;
...
if (variable_global_exists("element_conflict")) {
    ...
    can_play_left = is_lotos || (value1 == global.left_end || can_match_l) && (global.element_conflict[element1] != global.left_element || global.left_element == ELEMENT.LOTOS) || 
                    (value2 == global.left_end || can_match_l) && (global.element_conflict[element2] != global.left_element || global.left_element == ELEMENT.LOTOS);
                    
    can_play_right = is_lotos || (value1 == global.right_end || can_match_r) && (global.element_conflict[element1] != global.right_element || global.right_element == ELEMENT.LOTOS) || 
                     (value2 == global.right_end || can_match_r) && (global.element_conflict[element2] != global.right_element || global.right_element == ELEMENT.LOTOS);
}
...
if (show_face) {
    ...
    if (effect_blink_timer > 0) {
        ...
        draw_col = merge_color(draw_col, effect_blink_color, pulse);
    }
    
    // ЗОЛОТОЙ ЛОТОС
    if (is_lotos) {
        draw_col = c_yellow; // Базовый желтый для золота
        final_alpha *= 0.8;
    }
}
...
if (show_face) {
    ...
    var _spr = is_lotos ? spr_00 : sprite_index;
    draw_sprite_ext(_spr, image_index, x, y, 1, 1, image_angle, draw_col, final_alpha);
    
    // Б) ДВУХЦВЕТНАЯ ОТРИСОВКА СТИХИЙ
    if (!is_lotos && (element1 != ELEMENT.NONE || element2 != ELEMENT.NONE)) {
    ...
    // Рубашка
    var _back_col = c_white;
    if (effect_blink_timer > 0) {
        var pulse = 0.5 + 0.5 * sin(current_time * 0.01);
        _back_col = merge_color(c_white, effect_blink_color, pulse);
    }
    draw_sprite_ext(spr_00, 0, x, y, 1, 1, image_angle, _back_col, 1);
}
